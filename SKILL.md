...
description: Use when a user needs tutoring for Brazilian public-service law exams, including detailed theory briefings, 20-question practice, corrections that preserve complete questions and alternatives, organized study materials, native visual maps, interactive HTML or PDF apostilas, trail progress, edital analysis, simulados, or discursivas.
---
...
3. Ao abrir uma sessão principal, anunciar: preparação teórica obrigatória, 20 questões por sessão, revisão com perguntas completas, materiais organizados, progresso e apostila em Markdown, HTML interativo e PDF. Registrar a sessão como `in_progress`, sem elevar o progresso, e persistir `question_target: 20`, `theory_briefing_version: 1` e `question_content_version: 1`.
...
6. Se o aluno pedir os arquivos, informar primeiro a raiz `<trail_dir>` e depois os caminhos exatos: fonte canônica em `modulos/.../topicos/<primeiro-topico>/sessoes/`; painéis em `painel/`; materiais por tópico em `modulos/.../topicos/.../`; caderno consolidado em `materiais/caderno-de-questoes.md`; agenda em `revisoes/`; apostilas em `apostila/apostila.{md,html,pdf}`; e mapas nativos em `assets/mapas/`. Explicar que `apostila/apostila.html` abre diretamente no navegador. Só corrigir após receber as 20 respostas. Se o aluno abandonar explicitamente as restantes, corrigir apenas as respondidas, manter `in_progress` e não elevar o progresso. No fechamento concluído, registrar em cada um dos 20 blocos o `Tópico`, o **enunciado integral**, **todas as alternativas** apresentadas e, depois, a resposta e o feedback; acrescentar o diagnóstico agregado e gerar todas as saídas.
...
| Correção objetiva | Enunciado integral → todas as alternativas → resposta → resultado → fundamento → alternativas úteis → tipo de erro → prevenção → revisão |
